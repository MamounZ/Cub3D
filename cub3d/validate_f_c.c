...
/*   Updated: 2025/10/04 16:05:25 by mazaid           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	store_rgb_value(t_data *var, char *id, int index, int value)
{
	if (value < 0 || value > 255)
		return (0);
	if (ft_strcmp(id, "C") == 0)
		var->ceiling_rgb[index] = value;
	else
		var->floor_rgb[index] = value;
	return (1);
}
...
int	fill_rgb_values(t_data *var, char **rgb, char *id)
{
	...
	while (rgb[i] && split_len == 3)
	{
		if (ft_is_number(rgb[i]))
			...
			value = -1;
		if (!store_rgb_value(var, id, i, value))
			return (0);
		i++;
	}
	return (i);
...
int	validate_f_c(t_data *var, char *value, char *id)
{
	...
	if (!rgb)
		return (1);
	count = fill_rgb_values(var, rgb, id);
	if (count != 3)
		var->ceiling_rgb[0] = -1;
	free(value);
	ft_free_split1(rgb);
